// ═══════════════════════════════════════════════════════
//  NÄRMA HELADERAS — LÓGICA DE LA APLICACIÓN
//  No es necesario tocar este archivo para cambios
//  visuales o de catálogo. Solo para features nuevas.
// ═══════════════════════════════════════════════════════
...
(function aplicarConfig() {
  ...
  // Nombre de marca y empresa
  document.querySelectorAll('.js-marca').forEach(el => el.textContent = CONFIG.nombreMarca);
  document.querySelectorAll('.js-empresa').forEach(el => el.textContent = CONFIG.nombreEmpresa);
  document.querySelectorAll('.js-heladera').forEach(el => el.textContent = CONFIG.nombreHeladera);
  document.querySelectorAll('.js-iniciales').forEach(el => el.textContent = CONFIG.iniciales);
})();
...
// TODO sprint siguiente: reemplazar por Google Sheets API
const DB = {
  funcionarios: [],
...
let selectedItems = {};   // { id: true } — multi-selección
...
function actualizarConfirm() {
  ...
  document.getElementById('confirm-items-list').innerHTML = lista;
...
function confirmar() {
  ...
  ids.forEach(id => {
    ...
    DB.consumos.unshift({
      hora,
      nombre: currentFunc.nombre,
      cedula: currentFunc.cedula,
      item:   id,
      monto:  it.precio,
    });
...
function renderAdmin() {
  ...
  document.getElementById('cat-chart').innerHTML = Object.keys(catCount).length === 0
    ? `<div class="empty-state">Sin consumos aún. El gráfico se actualizará con el primer registro.</div>`
